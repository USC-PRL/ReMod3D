...
#include "ReBotsModuleGameMode.h"
 ...
 */
AManipulatorObjectActor* AReBotsModuleGameMode::GetManipObject() {
    return ManipObject;
}
/**
 ...
 */
APhysicsConstraintActor* AReBotsModuleGameMode::GetManipConstraint() {
 ...
 */
FTransform AReBotsModuleGameMode::GetCorrectiveDockingTransform(ModuleDockFace d1, ModuleDockFace d2) {
 ...
 */
void AReBotsModuleGameMode::DockModuleToGround(AModularRobot* m, ModuleDockFace d1) {
    ASuperBotModule *sm1 = (ASuperBotModule*)m;
    FName d1S = "BackDock";
 ...
 */
void AReBotsModuleGameMode::DockObjectToGround(AManipulatorObjectActor* a) {
 ...
 * @param a manipulator object actor
 */
void AReBotsModuleGameMode::DockStackableObjectToGround(AStackableObjectActor* a) {
    APhysicsConstraintActor *apc = GetWorld()->SpawnActor<APhysicsConstraintActor>();
    apc->GetConstraintComp()->SetConstrainedComponents(a->GetBox(), "Box", Floor->GetStaticMeshComponent(), "");
    apc->GetConstraintComp()->SetLinearXLimit(ELinearConstraintMotion::LCM_Locked, 0);
    apc->GetConstraintComp()->SetLinearYLimit(ELinearConstraintMotion::LCM_Locked, 0);
 ...
 */
void AReBotsModuleGameMode::DockModuleToObject(AModularRobot* m, ModuleDockFace d1,
 ...
 */
void AReBotsModuleGameMode::DockModuleToStackableObject(AModularRobot* m1, ModuleDockFace d1, AStackableObjectActor* actor) {
    ASuperBotModule *sm1 = (ASuperBotModule*)m1;
    FName d1S = "BackDock";
    if (d1 == DOCK_0) d1S = "FrontDock";
    else if (d1 == DOCK_1) d1S = "BackDock";
    else if (d1 == DOCK_2) d1S = "LeftDock";
 ...
 * @param d2 dock face of module 2
 */
void AReBotsModuleGameMode::DockTwoModules(AModularRobot* m1, AModularRobot* m2, ModuleDockFace d1, ModuleDockFace d2) {
    ASuperBotModule *sm1 = (ASuperBotModule*)m1;
    ASuperBotModule *sm2 = (ASuperBotModule*)m2;
    ...
    std::pair<ASuperBotModule*, ModuleDockFace> modPair(sm1, (ModuleDockFace)d1);
    std::pair<ASuperBotModule*, ModuleDockFace> modPair2(sm2, (ModuleDockFace)d2);
 ...
 * @param d2 dock face of module 2
 */
void AReBotsModuleGameMode::UnDockTwoModules(AModularRobot* m1, AModularRobot* m2, ModuleDockFace d1, ModuleDockFace d2) {
    ASuperBotModule *sm1 = (ASuperBotModule*)m1;
    ASuperBotModule *sm2 = (ASuperBotModule*)m2;
    typedef std::map<std::pair<AModularRobot*, ModuleDockFace>, APhysicsConstraintActor*>::iterator monitorIt;
    for (monitorIt iterator = InterModuleConstraints.begin(); iterator != InterModuleConstraints.end(); iterator++) {
        std::pair<AModularRobot*, ModuleDockFace> modPair = iterator->first;
        APhysicsConstraintActor* a = iterator->second;
        if ((modPair.first == sm1 && modPair.second == (ModuleDockFace)d1)) {
            if (a) {
 ...
 */
AReBotsModuleGameMode::AReBotsModuleGameMode(const FObjectInitializer& ObjectInitializer)
...
* @param a2 actor 2
*/
void AReBotsModuleGameMode::RemoveStackableConstraint(AActor* a1, AActor* a2) {
	APhysicsConstraintActor* c = StackConstraintsMap[std::pair<AActor*, AActor*>(a1, a2)];
	if (c) {
		c->GetConstraintComp()->BreakConstraint();
		c->Destroy();
		StackConstraintsMap[std::pair<AActor*, AActor*>(a1, a2)] = NULL;
		StackConstraintsMap[std::pair<AActor*, AActor*>(a2, a1)] = NULL;
	}
}
/**
 ...
 */
void AReBotsModuleGameMode::Tick(float DeltaSeconds) {
    ...
    if (FirstTick) {
        ...
        TActorIterator< AStackableObjectActor> StackableObjectIt = TActorIterator<AStackableObjectActor>(GetWorld());
        while (SuperbotIt) {
            ...
            modsToIDs[(AModularRobot*)(*SuperbotIt)] = resultInt;
            modulesMap[resultInt] = (AModularRobot*)(*SuperbotIt);
            ++SuperbotIt;
        }
        ...
        while (StackableObjectIt) {
            StackableObjects.push_back(*StackableObjectIt);
            ++StackableObjectIt;
        }
        
		TActorIterator< APhysicsConstraintActor> SConstraintIt = TActorIterator<APhysicsConstraintActor>(GetWorld());
		while (SConstraintIt) {
			APhysicsConstraintActor *c = *SConstraintIt;
			if (c->GetConstraintComp()->ConstraintActor1->GetClass() == AStackableObjectActor::StaticClass() ||
				c->GetConstraintComp()->ConstraintActor2->GetClass() == AStackableObjectActor::StaticClass()) {
				StackConstraints.push_back(c);
				StackConstraintsMap[std::pair<AActor*, AActor*>(c->GetConstraintComp()->ConstraintActor1, c->GetConstraintComp()->ConstraintActor2)] = c;
				StackConstraintsMap[std::pair<AActor*, AActor*>(c->GetConstraintComp()->ConstraintActor2, c->GetConstraintComp()->ConstraintActor1)] = c;
			}
			++SConstraintIt;
		}


		for (int i = 0; i < StackConstraints.size(); ++i) {
			for (int j = 0; j < StackConstraints.size() - i - 1; ++j)
			{
				if (StackConstraints[j]->GetTransform().GetLocation().Z <
					StackConstraints[j + 1]->GetTransform().GetLocation().Z)
				{
					APhysicsConstraintActor* temp = StackConstraints[j];
					StackConstraints[j] = StackConstraints[j + 1];
					StackConstraints[j + 1] = temp;
				}
			}
		}

		for (int i = 0; i < StackableObjects.size(); ++i) {
			for (int j = 0; j < StackableObjects.size() - i - 1; ++j)
			{
				if (StackableObjects[j]->GetTransform().GetLocation().Z <
					StackableObjects[j + 1]->GetTransform().GetLocation().Z)
				{
					AStackableObjectActor* temp = StackableObjects[j];
					StackableObjects[j] = StackableObjects[j + 1];
					StackableObjects[j + 1] = temp;
				}
			}
		}
        ...
        while (ConstraintIt) {
            ...
            if (c->GetConstraintComp()->ConstraintActor1->GetClass() == ASuperBotModule::StaticClass() &&
                ...
                if (sm1 && sm2) {
                    ...
                    InterModuleConstraints[std::pair<AModularRobot*, ModuleDockFace>((AModularRobot*)sm1, (ModuleDockFace)dock1)] = c;
                    InterModuleConstraints[std::pair<AModularRobot*, ModuleDockFace>((AModularRobot*)sm2, (ModuleDockFace)dock2)] = c;
    ...
    //Sort stackable objects in reverse order of their height from the ground
    for (int i = 0; i < StackableObjects.size(); ++i) {
        for (int j = 0; j < StackableObjects.size() - i - 1; ++j)
        {
            if (StackableObjects[j]->GetActorTransform().GetLocation().Z <
                StackableObjects[j + 1]->GetActorTransform().GetLocation().Z)
            {
                AStackableObjectActor* temp = StackableObjects[j];
                StackableObjects[j] = StackableObjects[j + 1];
                StackableObjects[j + 1] = temp;
 ...
 */
vector<AStackableObjectActor*> AReBotsModuleGameMode::GetStackableObjects() {
 ...
 */
void AReBotsModuleGameMode::sendMessageToDock(AModularRobot *mod, ModuleDock dock, std::string message) {
    ASuperBotModule* module = (ASuperBotModule*)mod;
    ...
    messageRoom[modsToIDs[connections[modsToIDs[module]][dock.GetDockFace()].GetModule()]].push_back(m);
}
/**
 ...
 * @param Mes message
 */
void AReBotsModuleGameMode::sendModuleMessageToDock(AModularRobot *sendingModule, ModuleDockFace Face, std::string Mes) {
 ...
 */
void AReBotsModuleGameMode::sendKinematicsMessageToDock(AModularRobot *mod, ModuleDock dock, KinematicsMessage mes) {
    ASuperBotModule* module = (ASuperBotModule*)mod;
    if (connections[modsToIDs[module]][dock.GetDockFace()].GetDockFace() == DOCK_6) {
        cout << "Message unroutable: module " << modsToIDs[module] << " dock " << dock.GetDockFace() << endl;
        cout << "No dock connected" << endl;
    ...
    messageRoomKinematics[modsToIDs[connections[modsToIDs[module]][dock.GetDockFace()].GetModule()]].push_back(mes);
}
/**
 *	@brief	Sends a kinematics message out on dock
 *	@param 	sendingModule 	module sending the message
 ...
 *	@param 	mes 	message contents
 */
void AReBotsModuleGameMode::sendTreeMessageToDock(AModularRobot *mod, ModuleDock dock, TreeMessage mes) {
    ASuperBotModule* module = (ASuperBotModule*)mod;
    if (connections[modsToIDs[module]][dock.GetDockFace()].GetDockFace() == DOCK_6) {
        cout << "Message unroutable: module " << modsToIDs[module] << " dock " << dock.GetDockFace() << endl;
        cout << "No dock connected" << endl;
    ...
    mes.SetSendingDock(dock);
    mes.SetReceivingDock(connections[modsToIDs[module]][dock.GetDockFace()]);
    messageRoomSuperBotTree[modsToIDs[connections[modsToIDs[module]][dock.GetDockFace()].GetModule()]].push_back(mes);
}
/**
 ...
 *	@param 	message 	message contents
 */
void AReBotsModuleGameMode::sendRangedMessage(AModularRobot *module, float range, string message) {
    ASuperBotModule* sendingModule = (ASuperBotModule*)module;
    ...
    for (int i = 0; i < (int)modulesMap.size(); i++) {
        if (modsToIDs[modulesMap[i]] != modsToIDs[sendingModule]) {
            FTransform pose1 = ASuperBotModule::SuperbotFix*((ASuperBotModule*)modulesMap[i])->GetMasterInnerMesh()->GetComponentTransform();
 ...
 */
Message AReBotsModuleGameMode::getLastMessage(int module) {
 ...
 *	@return	most recent message in module's queue of messages
 */
ModuleMessage AReBotsModuleGameMode::getLastModuleMessage(int module) {
 ...
 */
RangedMessage AReBotsModuleGameMode::getLastRangedMessage(int module) {
 ...
 */
vector<RangedMessage> AReBotsModuleGameMode::getRangedMessages(int module) {
 ...
 */
vector<Message> AReBotsModuleGameMode::getMessages(int module) {
 ...
 */
vector<ModuleMessage> AReBotsModuleGameMode::getModuleMessages(int module) {
 ...
 */
void AReBotsModuleGameMode::clearMessages(int module) {
    messageRoom[module].clear();
}
 ...
 *	@return	most recent message in module's queue of messages
 */
KinematicsMessage AReBotsModuleGameMode::getLastKinematicsMessage(int module) {
 ...
 *	@return	messages fromm module's message queue
 */
vector<KinematicsMessage> AReBotsModuleGameMode::getKinematicsMessages(int module) {
 ...
 *	@return	most recent message in module's queue of messages
 */
TreeMessage AReBotsModuleGameMode::getLastTreeMessage(int module) {
 ...
 *	@return	messages fromm module's message queue
 */
vector<TreeMessage> AReBotsModuleGameMode::getTreeMessages(int module) {
 ...
 */
void AReBotsModuleGameMode::RenderBehaviorData(AModularRobot *M, bool EngagedToGround, ModuleDockFace D) {
    ASuperBotModule *Mod = (ASuperBotModule*)M;
    ...
    StackToPlaneBehavior* STP = NULL;
	DistributedIKBehavior* DIK = NULL;
	DistributedKinematicsBehavior* DKB = NULL;
	if (!(Mod)->GetModuleBehavior()) return;
    ...
    if ((Mod)->GetModuleBehavior()->GetName().compare("StackToPlane") == 0) {
        STP = (StackToPlaneBehavior*)(Mod)->GetModuleBehavior();
    }
	if ((Mod)->GetModuleBehavior()->GetName().compare("DIK") == 0) {
		DIK = (DistributedIKBehavior*)(Mod)->GetModuleBehavior();
	}
	if ((Mod)->GetModuleBehavior()->GetName().compare("DistributedKinematics") == 0) {
		DKB = (DistributedKinematicsBehavior*)(Mod)->GetModuleBehavior();
	}
	...
	if (DKB) {
		if (DKB->GetAmILeader()) {
			FTransform TargTrans = DKB->GetTransform();
			FTransform GroundTrans = DKB->GetTransformOfGround()*TargTrans;
			if (!TargetTransformActor) {
				TargetTransformActor = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
			}
			else {
			...
			if (!GroundTransActor) {
				GroundTransActor = GetWorld()->SpawnActor<AReferenceFrameActor>(GroundTrans.GetLocation(), FRotator(GroundTrans.GetRotation()));
			}
			else {
				GroundTransActor->SetActorTransform(GroundTrans);
			}
			if (DKB->GetEndEffectorTransforms().size() == 2) {
				FTransform EETrans1 = DKB->GetEndEffectorTransforms()[0] * TargTrans;
				FTransform EETrans2 = DKB->GetEndEffectorTransforms()[1] * TargTrans;
				if (!EE1) {
					EE1 = GetWorld()->SpawnActor<AReferenceFrameActor>(EETrans1.GetLocation(),
						EETrans1.GetRotation().Rotator());
				}
				else {
					EE1->SetActorTransform(EETrans1);
				}
				if (!EE2) {
					EE2 = GetWorld()->SpawnActor<AReferenceFrameActor>(EETrans2.GetLocation(),
						EETrans2.GetRotation().Rotator());
				}
				else {
					EE2->SetActorTransform(EETrans2);
				}
			}
		}
	...
	if (DIK && EngagedToGround) {
		if (DIK->GetSuperBotTree().GetVertices().size() > 0) {
			int see = DIK->GetSelectedEndEffector();
			if (see != -1 && DIK->GetSuperBotTree().GetVertices().size() - 1 >= see) {
				FTransform FinalFKActorTrans = DIK->GetSuperBotTree().GetVertices()[see]->GetTransform()*
					SuperbotKinematics::computeTransformToDock((Mod)->GetJointAngles(), D)*
					ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
				ModuleMotorVector mvs(DIK->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[0],
					DIK->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[1],
					DIK->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[2]);
				FinalFKActorTrans = SuperbotKinematics::computeTransformToDock(mvs,
					DIK->GetEndEffectorDockFace())*FinalFKActorTrans;
				if (DIK->GetSuperBotTree().GetVertices()[see]->GetAmIConnectedToObject()) {
					if (DIK->GetEndEffectorDockFace() == DOCK_0) {
						FinalFKActorTrans = FTransform(FVector(25, 0, 0))*FinalFKActorTrans;
					}
					else {
						FinalFKActorTrans = FTransform(FVector(-25, 0, 0))*FinalFKActorTrans;
					}
    ...
    if (STP && EngagedToGround) {
        if (STP->GetSuperBotTree().GetVertices().size() > 0) {
            int see = STP->GetSelectedEndEffector();
            if (see != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= see) {
                FTransform FinalFKActorTrans = STP->GetSuperBotTree().GetVertices()[see]->GetTransform()*
                SuperbotKinematics::computeTransformToDock((Mod)->GetJointAngles(), D)*
                ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
                ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[0],
                                      STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[1],
                                      STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[2]);
                FinalFKActorTrans = SuperbotKinematics::computeTransformToDock(mvs,
                                                                               STP->GetEndEffectorDockFace())*FinalFKActorTrans;
                if (STP->GetSuperBotTree().GetVertices()[see]->GetAmIConnectedToObject()) {
                    if (STP->GetEndEffectorDockFace() == DOCK_0) {
                        FinalFKActorTrans = FTransform(FVector(25, 0, 0))*FinalFKActorTrans;
                    }
                    else {
                        FinalFKActorTrans = FTransform(FVector(-25, 0, 0))*FinalFKActorTrans;
        ...
        if (STP->GetBestTransforms().size() > 0) {
            int gm = STP->GetGroundModule();
            std::vector<float> sol = STP->GetSolutionPos();
            int see = STP->GetSelectedEndEffector();
            if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
                ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[1],
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
                ModuleMotorVector end(sol[3 * see + 0], sol[3 * see + 1], sol[3 * see + 2]);
                FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
                FTransform EndModuleTrans =
                SuperbotKinematics::computeTransformToDock(end, STP->GetEndEffectorDockFace())*
                STP->GetBestTransforms()[see] * GroundActorTrans;
                if (!TargetFootActor) {
                    //TargetFootActor = GetWorld()->SpawnActor<AReferenceFrameActor>((EndModuleTrans).GetLocation(), FRotator(EndModuleTrans.GetRotation()));
                }
                else {
		...
		if (STP->GetSuperBotTree().GetVertices().size() > 0) {
			...
			if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
				ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
					...
					STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
				FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
				//FTransform TargTrans = STP->ComputeObjectPose()*GroundActorTrans;
				if (STP->GetEndEffectorDockFace() == DOCK_0) {
					//TargTrans = FTransform(FQuat(FVector(0, 1, 0), M_PI), FVector(0, 0, 0))*TargTrans;
				}
				if (!CurrentObTarget) {
					//CurrentObTarget = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
				}
				else {
					//CurrentObTarget->SetActorTransform(TargTrans);
				}
			}
		}
		...
		if (STP->GetSuperBotTree().GetVertices().size() > 0) {
			...
			if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
				ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
					...
					STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
				FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
				//FTransform TargTrans =STP->GetStackableObs()[STP->GetCurrentStackableObjectIndex()]->GetActorTransform();
				//TargTrans.SetScale3D(FVector(1, 1, 1));
				if (!CurrentOb) {
					//CurrentOb = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
				}
				else {
					//CurrentOb->SetActorTransform(TargTrans);
				}
			}
		}
		...
		if (STP->GetSuperBotTree().GetVertices().size() > 0) {
			...
			if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
				ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
					...
					STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
				FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
				//FTransform TargTrans = STP->GetTargetOnGroundTransform()*GroundActorTrans;
				if (!TargetProjectedToGround) {
					//TargetProjectedToGround = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
				}
				else {
					//TargetProjectedToGround->SetActorTransform(TargTrans);
				}
			}
		}
		if (STP->GetSuperBotTree().GetVertices().size() > 0) {
			...
			if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
				ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
					...
					STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
				FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
				//FTransform TargTrans = STP->GetObjectOnGroundTransform()*GroundActorTrans;
				if (!ObjectProjectedToGround) {
					//ObjectProjectedToGround = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
				}
				else {
					//ObjectProjectedToGround->SetActorTransform(TargTrans);
				}
			}
		}
        if (STP->GetSuperBotTree().GetVertices().size() > 0) {
            ...
            if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
                ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
                                      ...
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
                FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
                //FTransform TargTrans = STP->GetTargetTransform()*GroundActorTrans;
                if (!TargetTransformActor) {
                    //TargetTransformActor = GetWorld()->SpawnActor<AReferenceFrameActor>(TargTrans.GetLocation(), FRotator(TargTrans.GetRotation()));
                }
                else {
        ...
        if (STP->GetSuperBotTree().GetVertices().size() > 0) {
            ...
            if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= gm) {
                ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
                                      ...
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
                FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
                FTransform ObjectTrans = STP->GetModuleBaseToObjectTransform()*GroundActorTrans;
                FTransform ActualGTrans = STP->GetModuleBaseToGroundTransform()*GroundActorTrans;
                ObjectTrans = FTransform(FVector(-50.0 / 2.0, 0, 0))*ObjectTrans;
                if (!ManipObjectTransformActor) {
                    //ManipObjectTransformActor = GetWorld()->SpawnActor<AReferenceFrameActor>(ObjectTrans.GetLocation(),
                    //	FRotator(ObjectTrans.GetRotation()));
		...
		if (STP->GetSuperBotTree().GetVertices().size() > 0 && false) {
            int gm = STP->GetGroundModule();
            std::vector<float> sol = STP->GetSolutionPos();
            int see = STP->GetSelectedEndEffector();
            if (gm != -1 && STP->GetSuperBotTree().GetVertices().size() - 1 >= see) {
                for (int i = 0; i < BoundingBoxes.size(); i++) {
                    if (BoundingBoxes[i])BoundingBoxes[i]->Destroy();
                    BoundingBoxes[i] = NULL;
                }
                ...
                ModuleMotorVector mvs(STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[0],
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[1],
                                      STP->GetSuperBotTree().GetVertices()[gm]->GetJointAngles()[2]);
                ModuleMotorVector end(STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[0],
                                      STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[1],
                                      STP->GetSuperBotTree().GetVertices()[see]->GetJointAngles()[2]);
                FTransform GroundActorTrans = SuperbotKinematics::computeTransformToDock(mvs, STP->GetGroundDockFace())*ASuperBotModule::SuperbotFix*(Mod)->GetMasterInnerMesh()->GetComponentTransform();
                for (int i = 0; i < STP->GetSuperBotTree().GetVertices().size(); i++) {
                    FTransform EndModuleTrans = STP->GetSuperBotTree().GetVertices()[i]->GetTransform()*GroundActorTrans;
                    FTransform MInnerEndModuleTrans = FTransform(FVector(30.0, 0.0, 0))*EndModuleTrans;
                    FTransform SInnerEndModuleTrans = FTransform(FVector(-30.0, 0.0, 0))*EndModuleTrans;
                    SInnerEndModuleTrans = FTransform(FQuat(FVector(1, 0, 0),
                                                            (M_PI / 180.0)*STP->GetSuperBotTree().GetVertices()[i]->GetJointAngles()[1]))*SInnerEndModuleTrans;
                    ABoundingBoxActor* B1 = GetWorld()->SpawnActor<ABoundingBoxActor>((MInnerEndModuleTrans).GetLocation(), FRotator(MInnerEndModuleTrans.GetRotation()));
                    B1->SetScale(FVector(0.6, 0.75, 0.75));
                    ABoundingBoxActor* B2 = GetWorld()->SpawnActor<ABoundingBoxActor>((SInnerEndModuleTrans).GetLocation(), FRotator(SInnerEndModuleTrans.GetRotation()));
                    B2->SetScale(FVector(0.6, 0.75, 0.75));
                    BoundingBoxes.push_back(B1);
                    BoundingBoxes.push_back(B2);
                    ModuleMotorVector currJoints(STP->GetSuperBotTree().GetVertices()[i]->GetJointAngles()[0],
                                                 STP->GetSuperBotTree().GetVertices()[i]->GetJointAngles()[1],
                                                 STP->GetSuperBotTree().GetVertices()[i]->GetJointAngles()[2]);
                    FTransform MOuterEndModuleTrans = SuperbotKinematics::computeTransformToDock(currJoints, DOCK_0)*
                    EndModuleTrans;
                    FTransform SOuterEndModuleTrans = SuperbotKinematics::computeTransformToDock(currJoints, DOCK_1)*
                    EndModuleTrans;
					...
					int objectTs = -1;
					for (int i = 0; i < STP->GetSuperBotTree().GetVertices().size(); i++) {
						if (STP->GetSuperBotTree().GetVertices()[i]->GetAmIConnectedToObject()) {
					...
					if (isObjectConnected) {
						ModuleDockFace LastDockFace = (ModuleDockFace)STP->GetSuperBotTree().GetVertices()[objectTs]->GetDockFacePath()[STP->GetSuperBotTree().GetVertices()[objectTs]->GetDockFacePath().size() - 1].first;
						...
						ModuleMotorVector currJoints(STP->GetSuperBotTree().GetVertices()[objectTs]->GetJointAngles()[0], 
							STP->GetSuperBotTree().GetVertices()[objectTs]->GetJointAngles()[1], 
							STP->GetSuperBotTree().GetVertices()[objectTs]->GetJointAngles()[2]);
						if (EEDockFace == DOCK_0) {
							obTrans = FTransform(FVector(20.0 / 1.8, 0, 0))*
								SuperbotKinematics::computeTransformToDock(currJoints, DOCK_0)*
								STP->GetSuperBotTree().GetVertices()[objectTs]->GetTransform() * GroundActorTrans;
						}
						else {
							obTrans = FTransform(FVector(-20.0 / 1.8, 0, 0))*
					...
					if (isObjectConnected) {
						ABoundingBoxActor* B9 = GetWorld()->SpawnActor<ABoundingBoxActor>((obTrans).GetLocation(), FRotator(obTrans.GetRotation()));
						B9->SetScale(FVector(.2, 2.5, 2.5));
						BoundingBoxes.push_back(B9);
					}
