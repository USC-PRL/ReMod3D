...
#include "SnakeIKBehavior.h"
///TODO: Add object to collision detection mechanism
/**
* @brief constructor
*/
SnakeIKBehavior::SnakeIKBehavior() {
	Init();
	this->Name = "SnakeIK";
...
*/
SnakeIKBehavior::SnakeIKBehavior(UWorld* W, std::string NameOfModule) {
	Init();
	this->Name = "SnakeIK";
	Seed = std::chrono::system_clock::now().time_since_epoch().count();
	Generator = std::mt19937(Seed);
...
*/
std::vector<float> SnakeIKBehavior::RandomCollisionFreeConfiguration(std::vector<std::pair<float,float> > bounds) {
...
*/
std::vector<std::pair<int, ModuleDockFace> > SnakeIKBehavior::Effectors(std::vector<float> config) {
...
*/
FTransform SnakeIKBehavior::GetModuleBaseToGroundTransform() {
...
*/
FTransform SnakeIKBehavior::GetModuleBaseToObjectTransform() {
...
*/
FTransform SnakeIKBehavior::GetMasterInnerTransformForBaseModule() {
	TActorIterator< ASuperBotModule > SuperbotIt = TActorIterator< ASuperBotModule >(World);
	while (SuperbotIt) {
...
*/
std::vector<std::pair<int, ModuleDockFace> > SnakeIKBehavior::GetTargetEffectors() {
...
*/
std::vector<FTransform> SnakeIKBehavior::GetTargetTransforms() {
...
*/
std::vector<float> SnakeIKBehavior::GetTargetAngles() {
...
*/
bool SnakeIKBehavior::CollisionFunction(std::vector<float> point) {
...
*/
bool SnakeIKBehavior::StateValid(std::vector<float> point) {
...
*/
bool SnakeIKBehavior::IsInSelfCollision(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) {
		Mods[i] = st.GetVertices()[i];
	}
...
* @return is position in self collision
*/
bool SnakeIKBehavior::IsInSelfCollisionDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
...
*/
bool SnakeIKBehavior::IsInCollisionWithGround(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
* @return is position in collision with ground
*/
bool SnakeIKBehavior::IsInCollisionWithGroundDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
*/
bool SnakeIKBehavior::PlanPathCurrentAnglesTo0Angles() {
	if (PPCounter == 0) {
		Path.clear();
	...
	if (HaveInitiatedPP && !HaveCompletedPP) {
		...
		RRTWorker = FRRTWorker::RRTInit(5000, st.GetVertices().size() * 3, 100, run, 10.0,
			std::bind(&SnakeIKBehavior::StateValid, this, std::placeholders::_1),
			std::bind(&SnakeIKBehavior::Distance, this, std::placeholders::_1,
				std::placeholders::_2),
...
*/
bool SnakeIKBehavior::FindGoalAnglesWithIK() {
	if (IKCounter == 0) {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		PSOWorker = FBBPSOWorker::BBPSOInit(50, 100, st.GetVertices().size() * 3, posB, velB, 4.0,
			std::bind(&SnakeIKBehavior::Fitness, this,
...
* @return true if process completed, false otherwise
*/
bool SnakeIKBehavior::PlanPathCurrentAnglesToGoalAngles() {
	if (PPCounter == 0) {
		Path.clear();
		HaveInitiatedPP = false;
		HaveCompletedPP = false;
...
* @return true if process completed false otherwise
*/
bool SnakeIKBehavior::ExecuteWait(int Counter) {
	if (EWCounter == 0) {
		WaitCounter = Counter;
	}
	if (WaitCounter <= 0) {
...
* @return true if process completed, false otherwise
*/
bool SnakeIKBehavior::ExecutePath() {
	if (PECounter == 0) {
		CurrentPathElement = 0;
		WaitCounter = 0;
...
*/
float SnakeIKBehavior::Distance(std::vector<float> point1, std::vector<float> point2) {
	float distance = 0.0;
...
*/
float SnakeIKBehavior::FitnessPSO(PSOParticle P, std::vector<float> start, std::vector<float> target) {
...
*/
float SnakeIKBehavior::GetDistanceToGroundScore(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
*/
std::vector<FTransform> SnakeIKBehavior::GetBestTransforms() {
...
*/
std::vector<float> SnakeIKBehavior::GetSolutionPos() {
	return SolutionPos;
}
/**
...
*/
vector<FTransform> SnakeIKBehavior::GetTransformsOfModulesForJointAngles(std::vector<ModuleNode*> Mods,
...
*/
float SnakeIKBehavior::Fitness(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	...
	for (int i = 0; i < TargetEffectors.size(); i++) {
		...
		if (TargetEffectors[i].first == SelectedEndEffector) DiffPosSize += (TargetEETransform.GetLocation() - FinalFKActorTrans.GetLocation()).Size();
		...
		if (TargetEffectors[i].first == SelectedEndEffector) DiffRotSize += OrientVec.Size();
	}
...
*/
FTransform SnakeIKBehavior::GetTargetTransform() {
...
* @param JAngles joint angles
*/
void SnakeIKBehavior::SetJointAnglesDistributed(std::vector<float> JAngles) {
...
*/
void SnakeIKBehavior::InitializeKinematicTree() {
...
*/
void SnakeIKBehavior::DiscoverKinematicStructure() {
...
*/
void SnakeIKBehavior::ProcessMessages(vector<TreeMessage> messages) {
...
*/
void SnakeIKBehavior::UpdateCurrentAngles() {
...
*/
void SnakeIKBehavior::UpdateDockEngagedStatus() {
...
* @brief update dock engaged status
*/
void SnakeIKBehavior::UpdateDockEngagedToGroundStatus() {
...
*/
void SnakeIKBehavior::UpdateDockEngagedToObjectStatus() {
...
*/
void SnakeIKBehavior::UpdateDockDiffs() {
...
*/
void SnakeIKBehavior::UpdateEngagedToGround() {
...
*/
void SnakeIKBehavior::UpdateEndEffectorStatus() {
...
*/
ModuleAction SnakeIKBehavior::GetKinematicsActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return Superbot action
*/
ModuleAction SnakeIKBehavior::GetTreeActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return path
*/
std::vector<int> SnakeIKBehavior::GetPathToRootFromNode(ModuleNode* n) {
	std::vector<int> path;
	ModuleNode* CNode = n;
...
*/
FTransform SnakeIKBehavior::GetCorrectiveDockingTransform(ModuleDockFace d1, ModuleDockFace d2) {
...
*/
ModuleTree SnakeIKBehavior::GetSuperBotTree() {
	return this->st;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace SnakeIKBehavior::GetEndEffectorDockFace() {
	return EEDock;
}
/**
* @brief get the selected end effector
*/
int SnakeIKBehavior::GetSelectedEndEffector() {
	return SelectedEndEffector;
}
/**
* @brief get the selected end effector
*/
int SnakeIKBehavior::GetGroundModule() {
	return this->GroundModule;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace SnakeIKBehavior::GetGroundDockFace() {
	return this->GDock;
}
/**
...
*/
bool SnakeIKBehavior::GetAmIEndEffector() {
...
*/
bool SnakeIKBehavior::GetAmIConnectedToGround() {
...
*/
bool SnakeIKBehavior::ShouldReInit() {
	return false;
}
...
*/
void SnakeIKBehavior::Tick(ModuleModel model) {
	...
	if (CanSendMessage && (CurrentDockEngagedToGroundStatus[0] || CurrentDockEngagedToGroundStatus[1])) {
		if (st.GetQueueSize() > 0) {
			...
			if (NumTests > 0) {
				...
				else if (!HaveFoundIKAngles) HaveFoundIKAngles = FindGoalAnglesWithIK();
