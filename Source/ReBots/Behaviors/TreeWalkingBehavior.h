...
class REBOTS_API TreeWalkingBehavior : public RobotModuleBehavior
{
	///Basic behavior data
...
public:
	...
	*/
	TreeWalkingBehavior(UWorld* W, std::string NameOfModule);
	/**
	* @brief destructor
	*/
	virtual ~TreeWalkingBehavior();
	...
	* @param tar target vector
	*/
	float FitnessRORT(PSOParticle P, std::vector<float> start, std::vector<float> target);
	/**
	* @brief transition valid long
	* @param s start
	* @param g goal
	* @return true if transition valid, false otherwise
	*/
	bool TransitionValidLong(std::vector<float> from, std::vector<float> to);
	/**
