...
#include "ReconfDemoQ.h"
///TODO: Add object to collision detection mechanism
/**
* @brief constructor
*/
ReconfDemoQBehavior::ReconfDemoQBehavior() {
	Init();
	this->Name = "ReconfQ";
...
*/
ReconfDemoQBehavior::ReconfDemoQBehavior(UWorld* W, std::string NameOfModule) {
	Init();
	this->Name = "ReconfQ";
	Seed = std::chrono::system_clock::now().time_since_epoch().count();
	Generator = std::mt19937(Seed);
...
*/
FTransform ReconfDemoQBehavior::GetModuleBaseToGroundTransform() {
...
*/
FTransform ReconfDemoQBehavior::GetModuleBaseToObjectTransform() {
...
*/
FTransform ReconfDemoQBehavior::GetMasterInnerTransformForBaseModule() {
	TActorIterator< ASuperBotModule > SuperbotIt = TActorIterator< ASuperBotModule >(World);
	while (SuperbotIt) {
...
*/
void ReconfDemoQBehavior::ConnectNewEffector() {
...
*/
void ReconfDemoQBehavior::ConnectObject() {
	cout << "Selected End Effector: " << SelectedEndEffector << endl;
...
*/
void ReconfDemoQBehavior::ReconnectObjectToGround() {
	cout << "Selected End Effector: " << SelectedEndEffector << endl;
...
*/
void ReconfDemoQBehavior::SwitchObject() {
...
*/
void ReconfDemoQBehavior::ReconfigureManipulator() {
	...
	for (int i = 1; i < P1.size(); i++) {
		if (DP1[2*i - 1].first == DOCK_3 && DP1[2*i].first == DOCK_1) {
	...
	for (int i = 1; i < P2.size(); i++) {
		if (DP2[2*i - 2].first == DOCK_3 && DP2[2*i - 1].first == DOCK_1) {
...
*/
bool ReconfDemoQBehavior::CollisionFunction(std::vector<float> point) {
	return IsInCollisionWithGroundDetailed(point) || IsInSelfCollision(point);
}
/**
* @brief state valid
* @param point point to check
...
*/
bool ReconfDemoQBehavior::StateValid(std::vector<float> point) {
...
*/
bool ReconfDemoQBehavior::IsInSelfCollision(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) {
		Mods[i] = st.GetVertices()[i];
	}
...
* @return is position in self collision
*/
bool ReconfDemoQBehavior::IsInSelfCollisionDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
...
*/
bool ReconfDemoQBehavior::IsInCollisionWithGround(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
* @return is position in collision with ground
*/
bool ReconfDemoQBehavior::IsInCollisionWithGroundDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
*/
bool ReconfDemoQBehavior::PlanPathCurrentAnglesTo0Angles() {
	if (PPCounter == 0) {
		Path.clear();
	...
	if (HaveInitiatedPP && !HaveCompletedPP) {
		...
		GEngine->AddOnScreenDebugMessage(PPProgressKey, 5.f, FColor::Red,
			FString::Printf(TEXT("Planning Path To 0")));
		if (HaveCompletedPP) {
		...
		PASOWorker = FPASOWorker::PASOInit(50, 50, st.GetVertices().size() * 3, 100, run, vel, 5.0,
			std::bind(&ReconfDemoQBehavior::FitnessPSO, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
			std::bind(&ReconfDemoQBehavior::StateValid, this, std::placeholders::_1),
			std::bind(&ReconfDemoQBehavior::Distance, this, std::placeholders::_1,
				std::placeholders::_2),
...
*/
bool ReconfDemoQBehavior::FindGoalAnglesWithIK() {
	if (IKCounter == 0) {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(20, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&ReconfDemoQBehavior::Fitness, this,
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::FindGoalAnglesForSwitch() {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(500, 1000, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&ReconfDemoQBehavior::FitnessSwitch, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::FindGoalAnglesForObject() {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			SolutionPos = vector<float>(st.GetVertices().size() * 3);
			PSOWorker = FBBPSOWorker::BBPSOInit(20, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&ReconfDemoQBehavior::FitnessObject, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::FindGoalAnglesForObjectSetDown() {
	if (ObjectIKCounter == 0) {
		HaveInitiatedIK = false;
		HaveCompletedIK = false;
	}
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			SolutionPos = vector<float>(st.GetVertices().size() * 3);
			PSOWorker = FBBPSOWorker::BBPSOInit(20, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&ReconfDemoQBehavior::FitnessObjectSetDown, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::FindGoalAnglesForReconfigure() {
	if (ReconfCounter == 0) {
		HaveInitiatedIK = false;
		HaveCompletedIK = false;
	}
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(30, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&ReconfDemoQBehavior::FitnessReconfigure, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::PlanPathCurrentAnglesToGoalAngles() {
	if (PPCounter == 0) {
		Path.clear();
		HaveInitiatedPP = false;
		HaveCompletedPP = false;
	...
	if (HaveInitiatedPP && !HaveCompletedPP) {
		GEngine->AddOnScreenDebugMessage(PPProgressKey, 5.f, FColor::Red,
			FString::Printf(TEXT("Planning Path To Goal")));
		HaveCompletedPP = PASOWorker->IsFinished();
		if (HaveCompletedPP) {
...
* @return true if process completed false otherwise
*/
bool ReconfDemoQBehavior::ExecuteWait(int Counter) {
	if (EWCounter == 0) {
		WaitCounter = Counter;
	}
	if (WaitCounter <= 0) {
...
* @return true if process completed, false otherwise
*/
bool ReconfDemoQBehavior::ExecutePath() {
	if (PECounter == 0) {
		CurrentPathElement = 0;
		WaitCounter = 0;
...
*/
float ReconfDemoQBehavior::Distance(std::vector<float> point1, std::vector<float> point2) {
	float distance = 0.0;
...
*/
float ReconfDemoQBehavior::FitnessPSO(PSOParticle P, std::vector<float> start, std::vector<float> target) {
...
*/
std::vector<FTransform> ReconfDemoQBehavior::GetBestTransforms() {
...
*/
std::vector<float> ReconfDemoQBehavior::GetSolutionPos() {
	return SolutionPos;
}
/**
...
*/
vector<FTransform> ReconfDemoQBehavior::GetTransformsOfModulesForJointAngles(std::vector<ModuleNode*> Mods,
...
*/
float ReconfDemoQBehavior::Fitness(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
...
* @return fitness value
*/
float ReconfDemoQBehavior::FitnessObject(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) { Mods[i] = st.GetVertices()[i]; }
...
* @return fitness value
*/
float ReconfDemoQBehavior::FitnessObjectSetDown(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) { Mods[i] = st.GetVertices()[i]; }
...
* @return fitness value
*/
float ReconfDemoQBehavior::FitnessSwitch(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	vector<int> EndEffectors;
...
* @return fitness value
*/
float ReconfDemoQBehavior::FitnessReconfigure(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	vector<int> EndEffectors;
...
*/
FTransform ReconfDemoQBehavior::GetTargetTransform() {
...
* @param JAngles joint angles
*/
void ReconfDemoQBehavior::SetJointAnglesDistributed(std::vector<float> JAngles) {
...
*/
void ReconfDemoQBehavior::InitializeKinematicTree() {
...
*/
void ReconfDemoQBehavior::DiscoverKinematicStructure() {
...
*/
void ReconfDemoQBehavior::ProcessMessages(vector<TreeMessage> messages) {
...
*/
void ReconfDemoQBehavior::UpdateCurrentAngles() {
...
*/
void ReconfDemoQBehavior::UpdateDockEngagedStatus() {
...
* @brief update dock engaged status
*/
void ReconfDemoQBehavior::UpdateDockEngagedToGroundStatus() {
...
*/
void ReconfDemoQBehavior::UpdateDockEngagedToObjectStatus() {
...
*/
void ReconfDemoQBehavior::UpdateDockDiffs() {
...
*/
void ReconfDemoQBehavior::UpdateEngagedToGround() {
...
*/
void ReconfDemoQBehavior::UpdateEndEffectorStatus() {
...
*/
ModuleAction ReconfDemoQBehavior::GetKinematicsActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return Superbot action
*/
ModuleAction ReconfDemoQBehavior::GetTreeActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return path
*/
std::vector<int> ReconfDemoQBehavior::GetPathToRootFromNode(ModuleNode* n) {
	std::vector<int> path;
	ModuleNode* CNode = n;
...
*/
FTransform ReconfDemoQBehavior::GetCorrectiveDockingTransform(ModuleDockFace d1, ModuleDockFace d2) {
...
*/
ModuleTree ReconfDemoQBehavior::GetSuperBotTree() {
	return this->st;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace ReconfDemoQBehavior::GetEndEffectorDockFace() {
	return EEDock;
}
/**
* @brief get the selected end effector
*/
int ReconfDemoQBehavior::GetSelectedEndEffector() {
	return SelectedEndEffector;
}
/**
* @brief get the selected end effector
*/
int ReconfDemoQBehavior::GetGroundModule() {
	return this->GroundModule;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace ReconfDemoQBehavior::GetGroundDockFace() {
	return this->GDock;
}
/**
...
*/
bool ReconfDemoQBehavior::GetAmIEndEffector() {
...
*/
bool ReconfDemoQBehavior::GetAmIConnectedToGround() {
...
*/
bool ReconfDemoQBehavior::ShouldReInit() {
	return false;
}
...
*/
void ReconfDemoQBehavior::Tick(ModuleModel model) {
	...
	if (CanSendMessage && (CurrentDockEngagedToGroundStatus[0] || CurrentDockEngagedToGroundStatus[1])) {
		if (st.GetQueueSize() > 0) {
			...
			if (!HaveFoundAnglesForReconfigure) HaveFoundAnglesForReconfigure = FindGoalAnglesForReconfigure();
