...
class REBOTS_API DistributedIKBehavior : public RobotModuleBehavior
{
	///Basic behavior data
	std::vector<float> TarVec; /**< target vector */
	bool CurrentlyWaiting = false; /**< currently waiting ? */
	int CurrentModule = 1; /**< current module */
	...
	AReferenceFrameActor* TarRef = NULL;
...
public:
	...
	* @brief constructor
	*/
	DistributedIKBehavior();
	/**
	* @brief constructor
	* @param W world
	* @param NameOfModule name of module
	*/
	DistributedIKBehavior(UWorld* W, std::string NameOfModule);
	/**
	* @brief destructor
	*/
	virtual ~DistributedIKBehavior();
