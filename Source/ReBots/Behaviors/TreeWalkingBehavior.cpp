...
#include "TreeWalkingBehavior.h"
///TODO: Add object to collision detection mechanism
/**
* @brief constructor
*/
TreeWalkingBehavior::TreeWalkingBehavior() {
	Init();
	this->Name = "TreeWalk";
...
*/
TreeWalkingBehavior::TreeWalkingBehavior(UWorld* W, std::string NameOfModule) {
	Init();
	this->Name = "TreeWalk";
	Seed = std::chrono::system_clock::now().time_since_epoch().count();
	Generator = std::mt19937(Seed);
...
*/
FTransform TreeWalkingBehavior::GetModuleBaseToGroundTransform() {
...
*/
FTransform TreeWalkingBehavior::GetModuleBaseToObjectTransform() {
...
*/
FTransform TreeWalkingBehavior::GetMasterInnerTransformForBaseModule() {
	TActorIterator< ASuperBotModule > SuperbotIt = TActorIterator< ASuperBotModule >(World);
	while (SuperbotIt) {
...
*/
void TreeWalkingBehavior::ConnectNewEffector() {
...
*/
void TreeWalkingBehavior::ConnectObject() {
	cout << "Selected End Effector: " << SelectedEndEffector << endl;
...
*/
void TreeWalkingBehavior::ReconnectObjectToGround() {
	cout << "Selected End Effector: " << SelectedEndEffector << endl;
...
*/
void TreeWalkingBehavior::SwitchObject() {
...
*/
bool TreeWalkingBehavior::CollisionFunction(std::vector<float> point) {
...
*/
bool TreeWalkingBehavior::StateValid(std::vector<float> point) {
...
*/
bool TreeWalkingBehavior::IsInSelfCollisionDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
...
* @return is position in self collision
*/
bool TreeWalkingBehavior::IsInSelfCollision(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) {
		Mods[i] = st.GetVertices()[i];
	}
...
*/
bool TreeWalkingBehavior::IsInCollisionWithGround(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
* @return is position in collision with ground
*/
bool TreeWalkingBehavior::IsInCollisionWithGroundDetailed(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
*/
bool TreeWalkingBehavior::PlanPathCurrentAnglesTo0Angles() {
	if (PPCounter == 0) {
		Path.clear();
	...
	if (HaveInitiatedPP && !HaveCompletedPP) {
		...
		if (!StateValid(start)) {
			...
			bool selfc = false;
			if (IsInSelfCollision(start)) {
				invalidString = "SelfCollision";
				selfc = true;
			}
			else {
				invalidString = "GroundCollision";
			}
			GEngine->AddOnScreenDebugMessage(StateValidKey, 5.f, FColor::Red, FString::Printf(TEXT("Start state INVALID: %d"), selfc));
		}
		else {
			GEngine->AddOnScreenDebugMessage(StateValidKey, 5.f, FColor::Green, FString("Start state VALID"));
		}
		//BestTransformsFound = GetTransformsOfModulesForJointAngles(Mods, SolutionPos);
		RRTWorker = FRRTWorker::RRTInit(10000, st.GetVertices().size() * 3, 100, run, 10.0,
			std::bind(&TreeWalkingBehavior::StateValid, this, std::placeholders::_1),
			std::bind(&TreeWalkingBehavior::Distance, this, std::placeholders::_1,
				std::placeholders::_2),
...
*/
bool TreeWalkingBehavior::FindGoalAnglesWithIK() {
	if (IKCounter == 0) {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(50, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&TreeWalkingBehavior::Fitness, this,
...
* @return true if process completed, false otherwise
*/
bool TreeWalkingBehavior::FindGoalAnglesForSwitch() {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(50, 100, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&TreeWalkingBehavior::FitnessSwitch, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool TreeWalkingBehavior::FindGoalAnglesForObject() {
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			SolutionPos = vector<float>(st.GetVertices().size() * 3);
			PSOWorker = FBBPSOWorker::BBPSOInit(50, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&TreeWalkingBehavior::FitnessObject, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool TreeWalkingBehavior::FindGoalAnglesForObjectSetDown() {
	if (ObjectIKCounter == 0) {
		HaveInitiatedIK = false;
		HaveCompletedIK = false;
	}
	...
	if (HaveInitiatedIK && !HaveCompletedIK) {
		...
		if (SelectedEndEffector != -1) {
			...
			PSOWorker = FBBPSOWorker::BBPSOInit(20, 50, st.GetVertices().size() * 3, posB, velB, 3.0,
				std::bind(&TreeWalkingBehavior::FitnessObjectSetDown, this,
					std::placeholders::_1, std::placeholders::_2, std::placeholders::_3),
				std::vector<float> {0, 200, 400, 100}, std::vector<float> {0, 200, 400, 100},
				SolutionPos, &SolutionFitness);
			HaveInitiatedIK = true;
...
* @return true if process completed, false otherwise
*/
bool TreeWalkingBehavior::PlanPathCurrentAnglesToGoalAngles() {
	if (PPCounter == 0) {
		Path.clear();
		HaveInitiatedPP = false;
		HaveCompletedPP = false;
...
* @return true if process completed false otherwise
*/
bool TreeWalkingBehavior::ExecuteWait(int Counter) {
	if (EWCounter == 0) {
		WaitCounter = Counter;
	}
	if (WaitCounter <= 0) {
...
* @return true if process completed, false otherwise
*/
bool TreeWalkingBehavior::ExecutePath() {
	if (PECounter == 0) {
		CurrentPathElement = 0;
		WaitCounter = 0;
...
*/
float TreeWalkingBehavior::Distance(std::vector<float> point1, std::vector<float> point2) {
	float distance = 0.0;
...
*/
float TreeWalkingBehavior::FitnessPSO(PSOParticle P, std::vector<float> start, std::vector<float> target) {
...
*/
float TreeWalkingBehavior::GetDistanceToGroundScore(std::vector<float> point) {
	vector<ModuleNode*> Mods(st.GetVertices().size());
	ModuleMotorVector mvsGround(st.GetVertices()[GroundModule]->GetJointAngles()[0],
		st.GetVertices()[GroundModule]->GetJointAngles()[1],
		st.GetVertices()[GroundModule]->GetJointAngles()[2]);
...
*/
std::vector<FTransform> TreeWalkingBehavior::GetBestTransforms() {
...
*/
std::vector<float> TreeWalkingBehavior::GetSolutionPos() {
	return SolutionPos;
}
/**
...
*/
vector<FTransform> TreeWalkingBehavior::GetTransformsOfModulesForJointAngles(std::vector<ModuleNode*> Mods,
...
*/
float TreeWalkingBehavior::Fitness(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
...
* @return fitness value
*/
float TreeWalkingBehavior::FitnessObject(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) { Mods[i] = st.GetVertices()[i]; }
...
* @return fitness value
*/
float TreeWalkingBehavior::FitnessObjectSetDown(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	for (int i = 0; i<st.GetVertices().size(); i++) { Mods[i] = st.GetVertices()[i]; }
...
* @return fitness value
*/
float TreeWalkingBehavior::FitnessSwitch(BranchBoundParticle P, std::vector<float> start, std::vector<float> target) {
	ModuleMotorVector mvsGround(P.GetPosition()[0], P.GetPosition()[1], P.GetPosition()[2]);
	FTransform BaseTransform = SuperbotKinematics::computeTransformToDock(mvsGround, GDock);
	vector<ModuleNode*> Mods(st.GetVertices().size());
	vector<int> EndEffectors;
...
*/
FTransform TreeWalkingBehavior::GetTargetTransform() {
...
* @param JAngles joint angles
*/
void TreeWalkingBehavior::SetJointAnglesDistributed(std::vector<float> JAngles) {
...
*/
void TreeWalkingBehavior::InitializeKinematicTree() {
...
*/
void TreeWalkingBehavior::DiscoverKinematicStructure() {
...
*/
void TreeWalkingBehavior::ProcessMessages(vector<TreeMessage> messages) {
...
*/
void TreeWalkingBehavior::UpdateCurrentAngles() {
...
*/
void TreeWalkingBehavior::UpdateDockEngagedStatus() {
...
* @brief update dock engaged status
*/
void TreeWalkingBehavior::UpdateDockEngagedToGroundStatus() {
...
*/
void TreeWalkingBehavior::UpdateDockEngagedToObjectStatus() {
...
*/
void TreeWalkingBehavior::UpdateDockDiffs() {
...
*/
void TreeWalkingBehavior::UpdateEngagedToGround() {
...
*/
void TreeWalkingBehavior::UpdateEndEffectorStatus() {
...
*/
ModuleAction TreeWalkingBehavior::GetKinematicsActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return Superbot action
*/
ModuleAction TreeWalkingBehavior::GetTreeActionForDock(ModuleDockFace d) {
	if (d == DOCK_0) {
...
* @return path
*/
std::vector<int> TreeWalkingBehavior::GetPathToRootFromNode(ModuleNode* n) {
	std::vector<int> path;
	ModuleNode* CNode = n;
...
*/
/*FTransform TreeWalkingBehavior::GetCorrectiveDockingTransform(ModuleDockFace d1, ModuleDockFace d2) {
...
*/
ModuleTree TreeWalkingBehavior::GetSuperBotTree() {
	return this->st;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace TreeWalkingBehavior::GetEndEffectorDockFace() {
	return EEDock;
}
/**
* @brief get the selected end effector
*/
int TreeWalkingBehavior::GetSelectedEndEffector() {
	return SelectedEndEffector;
}
/**
* @brief get the selected end effector
*/
int TreeWalkingBehavior::GetGroundModule() {
	return this->GroundModule;
}
/**
* @brief get the selected end effector
*/
ModuleDockFace TreeWalkingBehavior::GetGroundDockFace() {
	return this->GDock;
}
/**
...
*/
bool TreeWalkingBehavior::GetAmIEndEffector() {
...
*/
bool TreeWalkingBehavior::GetAmIConnectedToGround() {
...
*/
bool TreeWalkingBehavior::ShouldReInit() {
	return false;
}
...
*/
void TreeWalkingBehavior::Tick(ModuleModel model) {
	...
	if (CanSendMessage && (CurrentDockEngagedToGroundStatus[0] || CurrentDockEngagedToGroundStatus[1])) {
		if (st.GetQueueSize() > 0) {
			...
			if (true) {
				GEngine->AddOnScreenDebugMessage(CurrentXKey, 4.f, FColor::Yellow, FString::Printf(TEXT("Walking")));
