...
*/
class REBOTS_API FRORTWorker : public FRunnable
{
	static  FRORTWorker* Runnable;/**<  Singleton instance, can access the thread any time via static accessor, if it is active! */
	...
	std::function<float(PSOParticle, std::vector<float>, std::vector<float>)> FitnessFunc;
	int MaxNumStates; /**< num allowable states */
	float Delta; /**< delta */
	RORTTree RORT; /**< instance of the optimization procedure */
...
public:
	...
	*/
	FRORTWorker(int NumStates, int NDims, int Try,
	...
	* @param SFitnessValue solution fitness value
	*/
	static FRORTWorker* RORTInit(int NumStates, int NDims, int Try,
		std::vector<std::pair<float, float> > PBounds, float D,
		std::function<bool(std::vector<float>)> SV,
		std::function<float(std::vector<float>, std::vector<float>)> Dis,
		std::function<float(PSOParticle, std::vector<float>, std::vector<float>)> F,
